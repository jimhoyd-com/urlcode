...
import type {ExportHeader,LinkRow,LinkStoreCommand,LinkStoreReply,LinkStoreWorkerData} from './link-store.ts';
if(!parentPort)throw new Error('Link store worker requires a parent');
const port=parentPort;
const data=workerData as LinkStoreWorkerData; // trust boundary: set by openConnection
const post=(message: LinkStoreReply): void=>port.postMessage(message);
let db: DatabaseSync|undefined;
const error=(status: number,message: string): never=>{throw new HttpError(status,message);};
type Row=Record<string,SQLOutputValue>;
const row=(value: Row): LinkRow=>({...value,enabled:value.enabled===1} as LinkRow); // trust boundary: a urlcode_links row
const integer=(value: SQLOutputValue|undefined): number=>typeof value==='number'?value:Number(value);
try {
...
const store=(): DatabaseSync=>db ?? error(503,'Link store unavailable');
function get(collection: string,code: string): LinkRow|null {
...
let exporting: {collection: string|null}|null=null;
function exportEnd(): boolean {
  if(!exporting)return false;
  exporting=null;
  try{store().exec('ROLLBACK');}catch{/* A lost transaction is already finished. */}
  return true;
}
function exportBegin(collection: string|undefined): ExportHeader {
  const db=store();
...
function exportPage({afterCollection='',afterCode='',limit=100}: {afterCollection?: string|undefined; afterCode?: string|undefined; limit?: number|undefined}): LinkRow[] {
  const db=store();
...
port.on('message',({id,operation,args}: LinkStoreCommand)=>{ // trust boundary: posted by openConnection
  try {
    ...
    let value: LinkRow|LinkRow[]|true|null;
    if(operation==='get'){value=get(collection,linkCode(code));}
      ...
      value=transaction((): LinkRow|true|null=>{
        const db=store();
        if(operation==='create'){
