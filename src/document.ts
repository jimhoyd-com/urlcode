...
export function renderDocument(options:DocumentOptions):string {
 ...
 const appearance=options.theme?`<div class="ui-appearance" data-ui-appearance hidden>${icon('sun')}<label for="ui-theme">${label('theme.label','Appearance')}</label><select id="ui-theme" data-ui-theme><option value="system">${label('theme.system','System')}</option><option value="light">${label('theme.light','Light')}</option><option value="dark">${label('theme.dark','Dark')}</option></select></div>`:'';
