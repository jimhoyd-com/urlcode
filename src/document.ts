...
export interface DocumentOptions {title:string;trustedContent:string;layout?:'default'|'compact'|'application';presentation?:PresentationContext;scripts?:readonly {src:string;nonce:string;async?:boolean}[]}
...
export function renderDocument(options:DocumentOptions):string {
 ...
 return `<!doctype html><html lang="${escapeHtml(p?.lang??'en')}" dir="${p?.dir==='rtl'?'rtl':'ltr'}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(options.title)}</title>${p?.favicon?`<link rel="icon" href="${localUrl(p.favicon)}">`:''}<style>${stylesheet}:root{${variables}}</style></head><body>${p?.logo?`<img class="ui-logo" src="${localUrl(p.logo)}" alt="" width="120">`:''}<a class="ui-skip" href="#main">${escapeHtml(p?.text('nav.skip')??'Skip to content')}</a><main id="main" class="ui-document" data-layout="${options.layout??'default'}"><h1>${escapeHtml(options.title)}</h1>${options.trustedContent}</main>${tags}</body></html>`;
}
