...
export interface ArtifactTransport { release(tag:string):Promise<ReleaseAsset[]>; download(url:string):Promise<Uint8Array>; attest(path:string):Promise<void>; }
...
export const githubTransport:ArtifactTransport={
  ...
  async attest(path) { await new Promise<void>((resolveVerify,reject)=>{ const child=spawn('gh',['attestation','verify',path,'--repo',ARTIFACT_REPOSITORY,'--signer-workflow',ARTIFACT_WORKFLOW],{stdio:'ignore'}); child.on('error',()=>reject(new ConfigError('GitHub CLI with attestation support is required to verify extension artifacts'))); child.on('exit',code=>code===0?resolveVerify():reject(new ConfigError('GitHub attestation verification refused the extension artifact'))); }); },
};
async function verifiedAsset(assets:ReleaseAsset[], asset:string, transport:ArtifactTransport):Promise<Uint8Array> { const found=assets.filter(item=>item.name===asset); assert(found.length===1,`Extension release is missing or repeats ${asset}`); const bytes=await transport.download(found[0]!.url); const temporary=join(tmpdir(),`urlcode-attest-${process.pid}-${Math.random().toString(16).slice(2)}`); await writeFile(temporary,bytes,{flag:'wx'}); try { await transport.attest(temporary); return bytes; } finally { await rm(temporary,{force:true}); } }
export async function resolveCatalog(release:string, transport:ArtifactTransport=githubTransport):Promise<{catalog:Catalog;assets:ReleaseAsset[]}> { const assets=await transport.release(release); const bytes=await verifiedAsset(assets,'extensions-catalog.json',transport); return {catalog:parseCatalog(bytes,release),assets}; }
export async function installArtifact(project:string, release:string, artifactName:string, transport:ArtifactTransport=githubTransport):Promise<ExtensionLock> {
  ...
  const bytes=await verifiedAsset(assets,entry.asset,transport); await extractArtifact(bytes,entry,cachePath(project,entry.sha256));
