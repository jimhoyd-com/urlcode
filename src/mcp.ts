...
const definitions=[
 ...
 {name:'review_project',description:'Opt-in, read-only static review of the compiled project plus its own function/middleware source for a bounded set of patterns (hand-written body validation, manual cookie/session construction, module-scope mutable state, outbound network calls) that may duplicate a declarative capability, be an already-declared extension\'s job, be a genuine gap, or need human review. Executes no project code, reads no environment/secret, makes no network call.',properties:{target:text}},
];
