...
export function button(label:string,type:'submit'|'button'='submit'):string {if(!['submit','button'].includes(type))throw new Error('Invalid button');return `<button data-slot="button" type="${type}">${escapeHtml(label)}</button>`;}
