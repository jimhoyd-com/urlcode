...
import type { Asset, AssetResult, PolicyContext, PolicyRequest, PolicyShared, PolicySupport, TargetName } from '../types.ts';
...
export interface CompressionState {
  encodings: Coding[]; minBytes: number; types: string[]; matches: (header: string | undefined) => boolean; level: number | null;
  levels: Partial<Record<Coding, Levels>>; allowWithSecrets: boolean; precompressed: number;
}
export interface CompressionDescription { encodings: Coding[]; minBytes: number; types: number; level: number | null; precompressed: number }
interface Codec { dynamic: number; stored: number; map: (level: number) => number; compress: (body: Buffer, level: number) => Buffer }
...
const codecs: Record<Coding, Codec> = {
...
function eligibleAssets(asset: Asset | Map<string, Asset> | undefined): Asset[] {
  if (!asset) return [];
  return asset instanceof Map ? [...asset.values()] : [asset];
}

export async function compile(config: CompressionConfig, { route, shared }: PolicyContext): Promise<CompressionState> {
  ...
  const levels: Partial<Record<Coding, Levels>> = {};
  ...
  const state: CompressionState = { encodings, minBytes, types, matches: typeMatcher(types), level: level ?? null, levels, allowWithSecrets: config.allowWithSecrets === true, precompressed: 0 };
...
export function negotiate<C extends string>(acceptEncoding: string | null | undefined, encodings: readonly C[]): C | null {
...
function withVary(headers: HeaderPair[]): HeaderPair[] {
...
function etagFor(headers: HeaderPair[], coding: Coding, strong: boolean): HeaderPair[] {
...
export function onResponse(state: CompressionState, request: PolicyRequest, result: AssetResult): HandlerResult {
  ...
  const varied: AssetResult = { ...result, headers: withVary(headers) };
  ...
  let encoded: Buffer, strong: boolean;
  ...
  const out: AssetResult = { ...varied, headers: [...etagFor(varied.headers, coding, strong).filter(([k]) => k.toLowerCase() !== 'content-encoding'), ['content-encoding', coding]] };
...
export function describe(state: CompressionState): CompressionDescription {
...
export async function close(shared: PolicyShared | undefined): Promise<void> { if (shared) shared.compressionBytes = 0; }
