...
async function evaluate(entry, name, payload, timeoutMs = 5000, chain = []) {
  ...
  for (const item of chain) allow(item.source);
...
parentPort.on('message', async ({id,source,name,request,context,maxBytes,timeoutMs,chain=[],native}) => {
  try {
    ...
    if (value.nativeBody) {
      ...
      for (const key of new Set(native.headers.map(([k]) => k.toLowerCase()))) {
        ...
        const output = value.headers.filter(pair => Array.isArray(pair) && typeof pair[0] === 'string' && pair[0].toLowerCase() === key).map(pair=>pair[1]);
    ...
    parentPort.postMessage({id,status:value.status,headers:value.headers,body,nativeBody:value.nativeBody === true});
