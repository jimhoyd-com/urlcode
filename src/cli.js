...
const usage = `URLCode 0.1.0-alpha.1 — local/self-hosted runtime
  ...
  urlcode doctor
Dev loads .env.local and watches; serve does neither. JavaScript functions are trusted operator code.
`;
...
try {
  const { values, positionals } = parseArgs({ allowPositionals:true, options: {
    ...
    origin:{ type:'string' }, alias:{ type:'string' }, local:{ type:'boolean' }, help:{ type:'boolean', short:'h' },
  } });
  ...
  if (values.help || !command) print(usage);
    ...
    switch (command) {
      ...
      case 'validate': {
        const runtime = await createRuntime(values.project, { local:values.local });
      ...
      case 'test': {
        const result = await runProjectTests(values.project, { log:print });
        print(result); if (result.failed) process.exitCode = 1; break;
      }
      case 'doctor':
        print({ node:process.version, platform:process.platform, architecture:process.arch, runtime:'node-process', providers:[], license:'undecided' }); break;
      case 'dev': case 'serve': {
        ...
        const app = await startServer({ project:values.project, host:values.host, port,
          local:command === 'dev', watch:command === 'dev', origin:values.origin });
