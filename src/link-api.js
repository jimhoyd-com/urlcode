...
export async function startLinkApi({store,collection,token,host='127.0.0.1',port=3001}) {
  ...
  const server=http.createServer({maxHeaderSize:8192,headersTimeout:5000,requestTimeout:10000,keepAliveTimeout:5000},async(req,res)=>{
    ...
    const send=(status,value,headers={})=>{const payload=value===undefined?undefined:JSON.stringify(value);res.writeHead(status,{'content-type':'application/json','cache-control':'no-store','x-content-type-options':'nosniff',...headers});res.end(payload);};
    try {
  ...
  return {address:server.address(),async close(){const timeout=setTimeout(()=>server.closeAllConnections(),10000);timeout.unref();await new Promise(resolve=>server.close(resolve));clearTimeout(timeout);}};
}
