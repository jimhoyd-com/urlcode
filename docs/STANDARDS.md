...
|---|---|---|
| RFC 9110 HTTP Semantics | conforms (the HEAD `Content-Length` deviation found by the original audit is fixed; see §1) | [1](#1-rfc-9110-http-semantics) |
...
| §9.3.2 HEAD suppresses body | conforms | `prepareResponse` in `src/http-response.ts` drops the body for HEAD and 204/205/304 | `test/http.test.ts` (HEAD `/go` body `''`); `test/http-policy.test.ts` lines 16, 23; `test/vercel.test.ts:49` |
| §8.6 `Content-Length` on HEAD MUST equal what GET would send | conforms (fixed since the original audit) | `prepareResponse` in `src/http-response.ts` writes `result.contentLength ?? result.body?.length ?? 0`, measured on the result before the body is dropped for HEAD, so HEAD states the length GET would send. Assets and cache hits set `contentLength` themselves (`src/assets.ts`, `src/policies/cache.ts`); compression skips dynamic bodies on HEAD (`src/policies/compression.ts`) so the identity length is the stated value | `test/standards.test.ts` "HEAD states the length GET would send on declared and function responses" |
...
| §7.6.1 hop-by-hop headers never set by a handler | conforms | `forbiddenHeaders` in `src/http-response.ts` (connection, keep-alive, transfer-encoding, content-length, upgrade, trailer, proxy-authenticate, proxy-authorization, te) stripped in `prepareResponse`, refused in YAML by the `reserved` set in `compileHttp` (`src/http-policy.ts`), and on error answers in `errorResponse` | `test/http-policy.test.ts` "invalid response policy ..." |
| §5.6.2 field names are tokens; §5.5 no CTLs in values | conforms | `src/header-validation.ts:6-7` (`token` regex, `invalidValue = /[^\t -~-ÿ]/`), applied in `prepareResponse` (`src/http-response.ts`), `compileHttp` (`src/http-policy.ts`) and `compile` (`src/policies/security.ts`) | `test/header-validation.test.ts` all three tests (compared against `node:http` over 0x00–0x11F) |
...
| §15.4.5 304 header set | conforms | Assets keep `Content-Type`, `ETag`, `Last-Modified`, `Cache-Control`, `Accept-Ranges` (`src/assets.ts:87-89`); `Content-Length` omitted for 304 (`prepareResponse` in `src/http-response.ts`). `revalidate` keeps `etag, cache-control, cdn-cache-control, vary, last-modified, content-location, expires, date, content-type` (`src/policies/cache.ts:190`) | `test/policy-cache.test.ts` "revalidate answers 304 ..." (checks `etag`, `cache-control`, `content-type` on the 304) |
...
| §9.3 `Connection: close` on error answers | conforms | `writeError` in `src/http-response.ts`; `clientError` reply in `src/server.ts` | untested |
...
| HSTS / origin never derived from `X-Forwarded-Proto`/`Host` | conforms | `onResponse` in `src/policies/security.ts` uses the operator origin; `src/server.ts:151` | `test/policy-security.test.ts` "HSTS follows the request origin scheme" (`x-forwarded-proto: https` does not enable it) |
...
| Requirement | Status | Behavior | Test |
|---|---|---|---|
| §6.1 directive syntax `max-age=...; includeSubDomains` | conforms | the `oshp` table in `src/policies/security.ts`: `max-age=31536000; includeSubDomains` | `test/policy-security.test.ts` "HSTS follows the request origin scheme" |
| §7.2 MUST NOT send STS over non-secure transport | conforms | emitted only when the configured origin is `https:` (`onResponse` in `src/policies/security.ts`); never inferred from forwarded headers | same test (plain origin → absent; `x-forwarded-proto` ignored; `origin: https://...` → present) |
...
| Requirement | Status | Behavior | Test |
|---|---|---|---|
| §3 one `Set-Cookie` header per cookie, never folded | conforms | cookies collected separately in `prepareResponse` and written as an array in `writeResponse` (`src/http-response.ts`); YAML `Set-Cookie` is the only list-valued header (`compileHttp` in `src/http-policy.ts`); AWS format 2.0 `cookies` array (`src/aws.ts`) | `test/http.test.ts` "function Request/Response ABI, scoped bindings, cookies ..."; `test/aws.test.ts` "cookies arrive through the format 2.0 array and leave through it" |
...
| Requirement | Status | Behavior | Test |
|---|---|---|---|
| OSHP "best practices" values | conforms to the pinned table | the frozen `oshp` table in `src/policies/security.ts`, whose comment names the revision (OSHP 2024 best practices); the tables in `docs/policies/security.md` are copied from the exported `profiles` constant; `X-Content-Type-Options: nosniff` supplied by the runtime on every response (`prepareResponse` and `errorResponse` in `src/http-response.ts`) rather than the profile. The upstream JSON could not be fetched from the audit environment (proxy 403), so the table is verified against the code and docs, not re-verified against owasp.org | `test/policy-security.test.ts` "profile tables are frozen, ordered and consistent with each other", "profile headers land on redirects, declared responses and function results; existing headers win" |
| fill-gaps semantics; `set`/`unset` | conforms | `compile` and `onResponse` in `src/policies/security.ts` | "set overrides the profile and existing headers; unset drops a profile header" |
| applied to errors and early denials | conforms | `src/runtime.ts` error path; `errorResponse` in `src/http-response.ts` | `test/policies.test.ts` "error responses carry the security headers of the matched route or the project"; `test/policy-security.test.ts` "early results that skip the handler still carry the profile" |
...
| Requirement | Status | Behavior | Test |
|---|---|---|---|
| CSP3 serialized policy syntax | conforms | `default-src 'self'; form-action 'self'; object-src 'none'; frame-ancestors 'none'; upgrade-insecure-requests; block-all-mixed-content` (the `oshp` table in `src/policies/security.ts`) | `test/policy-security.test.ts` (value equality with the frozen table) |
...
1. **HEAD `Content-Length: 0` on `respond`, function and middleware results** (RFC 9110 §8.6 MUST NOT; §1). Fixed: `prepareResponse` in `src/http-response.ts` measures the length on `result.body` before it is dropped for HEAD, covered by `test/standards.test.ts`; the Cloudflare and AWS parity tests cover the adapters.
...
- A change to the OSHP table updates the revision comment above the `oshp` table in `src/policies/security.ts`, updates the tables in `docs/policies/security.md` to match the `profiles` constant, and must keep `test/policy-security.test.ts` "profile tables are frozen, ordered and consistent" passing; update §17–§18.
