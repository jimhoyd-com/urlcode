...
> **Never published.** 0.4.3 was prepared, but its candidate build failed in the release container (two checks called `git ls-files` on a checkout git treated as dubious ownership), and a source change needs a new version. Nothing was tagged or published for 0.4.3; everything listed here ships in 0.4.6 (0.4.4 and 0.4.5 were also never published).
