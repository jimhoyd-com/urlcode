...
SQLite operations run in a dedicated worker, separate from function workers and
the HTTP event loop. One store connection admits up to 32 outstanding operations,
with a 5-second deadline including waiting. SQLite lock wait is bounded to one
second. Excess work fails with 503; there is no unbounded request queue. A worker
failure makes the store unavailable until reload/restart; no automatic reconnect
loop is promised. Plain YAML redirects can continue serving.
