...
| `RIM-OUTPUT-001` | `packages/core/src/http-response.ts`: `prepareResponse`, `errorResponse` | Produce a bounded HTTP response, preserve separate cookies, remove hop-by-hop headers, and suppress a HEAD body without changing handler selection. | [Functions](SPECIFICATION.md#functions), [HTTP](HTTP.md), `test/http.test.ts` |
