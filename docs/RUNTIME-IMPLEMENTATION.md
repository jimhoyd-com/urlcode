...
| `RIM-CAP-001` | `packages/core/src/capabilities.ts`: `analyzeProjectCapabilities`, `assertTargetCompatibility` | Inspect the fully normalized project and refuse each unsupported capability with its route named before activation. | [Capabilities](CAPABILITIES.md), `test/capabilities.test.ts` |
...
| `RIM-PATTERN-001` | `packages/core/src/pattern-guard.ts`: `assertSafePattern`, `backtrackingPaths`; `packages/core/src/policies/agents.ts`: `validatePattern`, `onRequest` | Admit an author regex (parameter and body `pattern`; agents `denyPatterns`, `allowPatterns` and list entries) only when its worst-case matching cost is bounded: refuse repeated quantified groups, lookaround and backreferences, cap unbounded quantifiers, charge every variable-width quantifier (`*`, `+`, `?`, `{n,}`, `{n,m}` with `m > n`) and alternation against one backtracking-path budget, and bound the input (128 characters for parameters and bodies; only the first 512 bytes of `User-Agent` are matched). Refuse the same patterns on every target, even one with a linear-time engine, so a project validates identically everywhere. | [HTTP](HTTP.md), [agents policy](policies/agents.md#the-pattern-subset), `test/body-validation-limits.test.ts`, `test/policy-agents.test.ts` |
...
| `RIM-GUEST-001` | `packages/core/src/trusted-functions.ts`: `TrustedFunctions`; `packages/core/src/functions.ts`: `FunctionPool` | Keep trusted Node execution and `sandbox: true` execution distinct. Do not substitute one for the other, and do not claim either is portable to an unrelated host. | [Functions](SPECIFICATION.md#functions), [function security](FUNCTION-SECURITY.md), `test/trusted-functions.test.ts`, `test/sandbox.test.ts` |
| `RIM-EXT-BUNDLE-001` | `packages/core/src/extension-bundles.ts`: `installBundle`, `loadExtensionBundle`; `packages/core/src/init-with.ts`: bundle distribution | Treat an executable extension bundle as trusted operator code, never project-selected code: verify the exact immutable GitHub tag attestation, catalog pin, digest, bounded archive and locked cache before loading its declared module entry. `init --with --bundle-release` may obtain scaffolds only from that verified temporary staging cache, then writes the lock/cache and explicit host bindings; it never turns YAML into a release selector or npm extension dependency. Refuse an incompatible core version before import. Preserve the separate `sandbox: true` route contract unchanged. | [Extensions](EXTENSIONS.md), [function security](FUNCTION-SECURITY.md), `test/extension-bundles.test.ts`, `test/init-with.test.ts` |
