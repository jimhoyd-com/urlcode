...
| `RIM-EXT-BUNDLE-001` | `packages/core/src/extension-bundles.ts`: `installBundle`, `loadExtensionBundle`; `packages/core/src/extension-artifacts.ts`: `installArtifact`; `packages/core/src/release-train.ts`: `resolveSafeReleaseTrain`; `packages/core/src/extension-transport.ts`: `verifiedReleaseAsset`, `peekCatalogCommit`; `packages/core/src/init-with.ts`: bundle distribution | Treat an executable extension bundle (and a declarative extension artifact) as trusted operator code/data, never project-selected: verify the exact immutable GitHub tag attestation, digest, bounded archive and locked cache before loading its declared module entry. A core-owned, tag-bound safe train may recommend one tested bundle/artifact pair only after its own attestation is verified against the dedicated release-train workflow and exact core version; it is discovery metadata, while project locks retain the resolved immutable catalog tags, commits and archive hashes. A catalog's own `commit` field is not merely recorded -- it is bound to that catalog attestation's cert-derived `--source-digest` (peeked ahead of the catalog's full parse) before the field is trusted, and every subsequent asset attestation for that release is bound to the same confirmed commit; a mismatch fails closed (#577). `init --with --bundle-release` may obtain scaffolds only from that verified temporary staging cache, then writes the lock/cache and explicit host bindings; it never turns YAML into a release selector or npm extension dependency. Refuse an incompatible core version before import. Preserve the separate `sandbox: true` route contract unchanged. A package may publish a second, separately named catalog entry that locks a different, non-host-activation module (for example `ui-presentation` locking `ui`'s root export instead of its `host/index.js`); such an entry is signed, versioned and integrity-locked exactly like any other bundle name and is never treated as scaffoldable by `init --with` unless its `scaffold` export actually names it (#522). | [Extensions](EXTENSIONS.md), [function security](FUNCTION-SECURITY.md), `test/extension-bundles.test.ts`, `test/extension-artifacts.test.ts`, `test/release-train.test.ts`, `test/init-with.test.ts`, `test/workspace-scaffold.integration.ts` |
| `RIM-OUTPUT-001` | `packages/core/src/http-response.ts`: `prepareResponse`, `errorResponse`, `writeResponse`, `writeError`; `packages/core/src/server.ts`: `contentLengthEnforcementIsSafe` | Produce a bounded HTTP response, preserve separate cookies, remove hop-by-hop headers, and suppress a HEAD body without changing handler selection. Content-Length is always the UTF-8 byte length of the body sent; a handler-stated length is honoured only on HEAD, where it reports what GET would send. A header repeated in the assembled result is written as separate wire lines, never collapsed to only its last value; the two runtime-owned singleton headers (`X-Content-Type-Options`, `X-Request-Id`) are the one exception, always exactly one line each regardless of what a handler set. Every `node:http`-backed writer (`server.ts`, `vercel.ts`) also asks Node itself to enforce that stated length (`res.strictContentLength`) as a belt-and-suspenders self-check over the length this module already computed -- except on Node 22.13.0-22.14.x, a documented-floor range (`engines`: `>=22.13.0`) where that Node self-check is itself broken and throws `ERR_HTTP_CONTENT_LENGTH_MISMATCH` from a byte-for-byte-correct response (urlcode#645); `contentLengthEnforcementIsSafe` turns Node's redundant check off only on that narrow range, never changing the Content-Length this module states or sends. | [Functions](SPECIFICATION.md#functions), [HTTP](HTTP.md#responses), `test/http.test.ts`, `test/response-length.test.ts` |
