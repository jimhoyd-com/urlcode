...
- Live short-code records need durable storage core does not have. The retired
  `urlcode-dynamic-link` extension is not a supported path; declare them through
  the operator-installed `store` extension's `extensions.store.config.shortLinks`
  ([data store](STORE.md)). General session/application storage remains future
  work.
