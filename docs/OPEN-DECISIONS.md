...
| Expand into business applications now? | No collection handler or proposed business suite is implemented; the model-backed benchmark evidence is missing. | Measure existing tasks and record repeated application plumbing before selecting a collection/CMS/forms project. Retired short-link products stay retired. Internal exploration is not a public implementation proposal. |
...
| Should `renderDocument` stop inlining its stylesheet? ([#287](https://github.com/jimhoyd-com/urlcode/issues/287), open) | `renderDocument` emits an inline `<style>`, which `oshp` (`default-src 'self'`) blocks; kit pages already link a hashed stylesheet and send their own nonce CSP. The PR for #287 adds `style: {nonce}` and `documentContentSecurityPolicy(nonce)` and documents the recipe. `oshp` is unchanged. | Recommendation: keep `oshp` strict and do not add a global `style-src` allowance or a named kit preset in core. Longer term, have `renderDocument` link a served hashed stylesheet like the kit does, which needs a host route and is a larger API change; decide after seeing whether the nonce recipe is enough. Not changed here. |
...
| Add an optional `default` to `{env: NAME}`? ([#258](https://github.com/jimhoyd-com/urlcode/issues/258), open) | `{env: NAME}` has no default and an unset or ungranted binding refuses to activate. The maintainer rejected `{value: default, from: host}`: a default a host can silently replace changes what a reviewed `urlcode.yaml` means. The data-directory pattern is documented and runnable ([YAML guide](yaml/organization.md#12-environment-and-secret-references), [`examples/data-dir`](../examples/data-dir/README.md)). Evidence for a default is anecdotal (n=1 each). | Deferred until more runs show the need. If it holds: an optional `default` on the operator-resolved `{env: NAME}`, so the override stays an operator-policy grant, is listed by `audit`, and a missing policy fails closed unless a default is declared. Not changed here. |
...
| [#253](https://github.com/jimhoyd-com/urlcode/issues/253) | Build declarative persistence as an operator-installed `store` extension, not core. | Ships as `@jimhoyd/urlcode-store` with `init --with store` and a [CRUD recipe](STORE.md). The retired short-link extension did not belong; it is no precedent against this (maintainer, #253). The `store-crud` catalog entry now exists (#324; no core change was needed: the fixture host already runs extension-backed recipes, and the test registers the real store). Not yet done: release-train wiring (`scripts/release*.ts`, workflow, package-audit peers) so it publishes with the others, filtering/sorting, per-record ownership, a SQLite backend. |
