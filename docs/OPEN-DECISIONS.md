...
| Add `respond.html`, or document `page` as the path? ([#278](https://github.com/jimhoyd-com/urlcode/issues/278), open) | `respond` accepts only `status`, `text` and `json` (`schemas/urlcode.schema.json`, `additionalProperties: false`, `text` and `json` mutually exclusive); `src/http-policy.ts` compiles it to a fixed reply with `text/plain; charset=utf-8` (or JSON) and a 1 MiB cap. `page` sends one prebuilt file inline as `text/html`, and `static`/`page`/`download` reject `response.headers` overrides of content type and cache control, so asset metadata has one owner. The reply gets `x-content-type-options: nosniff`, and the `oshp` profile adds a CSP (`default-src 'self'`), so inline `<script>`/`<style>` in a declared page would be blocked either way. No escaping is involved: the body is a fixed string, never interpolated with request data. Also verified by reading, not run: `respond` plus a `response.headers` `content-type: text/html` already replaces the default (`decorateResponse`), so an undocumented one-file workaround exists. Evidence is one n=1 benchmark (8 tool calls, 6 of them discovery). | Recommendation: do not add `respond.html`; document `page` as the intended path for HTML and put the one-line HTML answer in the `init` starter and `recipes`, because the cost seen was discovery, not a missing feature. Reasons: a second HTML path would split `audit`/`explain` (explain reports `page` as a file with a content type; an inline body has no file to hash or lint), invite HTML inside YAML strings that the CSP then breaks, and duplicate content-type ownership that `page` already centralizes. Also decide whether the `respond` + `content-type: text/html` workaround stays allowed or is rejected with a pointer to `page`; recommend rejecting so the rule is uniform (unverified that no test or example depends on it). What would change this: repeated benchmark or user runs where the two-file cost, not discovery, blocks a task; or evidence that agents commonly hit the workaround. If added, it must mirror `text` (1 MiB cap, no 204/205 body, fixed `text/html; charset=utf-8`), be mutually exclusive with `text` and `json`, and land in the schema, catalog, context, `explain` and tests together. Not implemented here. |
...
| Multi-step fixtures, captured values, a restart step and an in-process test helper? ([#256](https://github.com/jimhoyd-com/urlcode/issues/256), open) | `tests/requests.json` fixtures are independent single requests against one server started by `runProjectTests`; nothing is captured between cases and the server is never restarted, so a create -> read -> update -> restart lifecycle needs a hand-written `node:test` harness. Evidence is two anecdotal benchmark runs (n=1 each). Quiet-by-default output with `--verbose` is done here. | Recommendation: add ordered `steps` to a fixture with `capture` (from JSON body or header) and `{{name}}` substitution, plus a `restart` step that closes and restarts the runtime on the same project and data directory; export the existing `startServer({port:0})` with a data-isolation option as the in-process helper. Not implemented: the fixture schema, data-isolation semantics for `restart` and the compatibility of `--target` deployment checks (which replay fixtures against a live deployment where restart is meaningless) need a decision first. |
