...
| Filtering and sorting for store collections and screens ([#330](https://github.com/jimhoyd-com/urlcode/issues/330)) | Recommended: the store gains declared, bounded query support, no arbitrary expressions. A collection declares `sortable` and `filterable` field lists; the list API accepts `sort=<field>` or `sort=-<field>` (one field, declared only) and `<field>=<value>` equality filters (declared only, at most a few per request, values typed like the field), with the existing page limit and a stable tie-break on id. Unknown or undeclared names are 400s naming the key. No operators, no free-text search, no OR, no nested paths. The screen then offers only the declared fields as sort choices and filter inputs and sends them as the same query parameters, so the API stays the single enforcement point. Build it in the store first; ranges and text search only after real demand. |
| Store record ownership ([#331](https://github.com/jimhoyd-com/urlcode/issues/331)) | Recommended, not implemented: a generic, opaque, core-validated principal handoff from an auth extension; declared `owner`, `owner-write` and `managed` modes enforced in the store with uniform 404s; no multi-user-safety claim until the cross-user denial suite passes. Questions and phases in [store ownership](STORE-OWNERSHIP.md). |
