...
| [#253](https://github.com/jimhoyd-com/urlcode/issues/253) | Build declarative persistence as an operator-installed `store` extension, not core. | Ships as `@jimhoyd/urlcode-store` with `init --with store` and a [CRUD recipe](STORE.md). The retired short-link extension did not belong; it is no precedent against this (maintainer, #253). Not yet done: a `recipes/` catalog entry, release-train wiring (`scripts/release*.ts`, workflow, package-audit peers) so it publishes with the others, filtering/sorting, per-record ownership, a SQLite backend. |
