...
cases) are supported by self-hosted/AWS/Vercel and refused by Cloudflare
(no artifact lowering yet) and by `static` (no server to match a request
against). See [egress](EGRESS.md) and [conditions](CONDITIONS.md).
...
`function`/`middleware`/`link`. See [Cloudflare](CLOUDFLARE.md), [AWS](AWS.md),
