...
identities and external publishing configuration bind to them.
...
flowchart LR
  ...
  main --> bundles --> bundleApproval --> bundleTag --> bundleRelease
  main --> artifactTag --> artifacts --> artifactApproval --> artifactRelease
```

### Operator entry points and gates
...
untouched. It then runs validation/tests (not `audit` or `benchmark`: the bare
starter ships with zero example routes by design, and both refuse to run
against a project with no active routes), and opens a resumable PR. The coordinator waits for checks and merges it, checking
...
<!-- urlcode-current-version:start -->
```sh
npm run release:template -- --version 0.5.9 --execute
```
<!-- urlcode-current-version:end -->
