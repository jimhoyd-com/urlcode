...
operator, credentials and provider tokens stay out of guest function/middleware
reach regardless of a route's trust mode, and every deployment target
...
- **Credentials stay out of guest reach.** Whether a route runs trusted
  (default) or with `sandbox: true`, functions never see raw credentials,
  password hashes, session secrets or provider tokens through this plugin.
  They get a narrow, grantable binding that answers "who is this and what may
  they do".
- **One store, every target.** Accounts, sessions and credentials live in
  the project's primary store, the one the link store (since removed from core,
  and retired) already was, with its export and restore discipline, through one interface
...
middleware seam. <!-- trust-model-prose: historical --> The one install step outside YAML is the plugin line in
