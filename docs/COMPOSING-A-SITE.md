...
site's dedicated `ui/` presentation directory, beside `host.mjs`. Paths retain
the `ui/` prefix and must remain inside that directory after symlink resolution.
The `ui/` directory itself cannot be a symlink. Keep only publishable presentation
content there: stylesheets become public assets. Operator source, data and secrets
belong outside it. Existing custom paths outside `ui/` must be moved under `ui/`
and their configuration updated; activation and `urlcode-ui doctor` reject them.
Nothing here forks a package.
