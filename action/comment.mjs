...
const notice = (/** @type {string} */ message) => console.log(`::notice::${message}`);
...
const api = async (/** @type {string} */ path, /** @type {RequestInit} */ init = {}) => fetch(`${GITHUB_API_URL}${path}`, { ...init, headers: { authorization: `Bearer ${GITHUB_TOKEN}`, accept: 'application/vnd.github+json', 'x-github-api-version': '2022-11-28', 'content-type': 'application/json', ...init.headers } });
try {
  let existing;
  for (let page = 1; page <= 10 && !existing; page++) {
    ...
    /** @type {{ id: number, body?: string, html_url?: string }[]} */
