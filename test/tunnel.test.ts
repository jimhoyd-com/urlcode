...
async function agent(t, body, { status = 200 } = {}) {
  ...
  return `http://127.0.0.1:${server.address().port}/api/tunnels`;
}
const tunnel = (publicUrl, addr, proto = 'https') => ({public_url:publicUrl,proto,config:{addr}});
// Never spawnSync here: the script fetches from a server in this process.
const run = (api, env = {}) => new Promise(resolve => {
...
test('an unreachable, empty or malformed agent fails with a usable message',{skip:windows && 'POSIX shell script'},async t => {
  for (const [body,options] of [[{tunnels:[]},{}],[{},{}],['not json',{}],[{tunnels:[]},{status:502}]]) {
