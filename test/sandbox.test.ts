...
test('guest has no Node, filesystem, shell, network or worker capabilities', async t => {
  const root = await project(t,{'/':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`export default () => Response.json({process:typeof process,require:typeof require,fetch:typeof fetch,WebSocket:typeof WebSocket,Worker:typeof Worker,Buffer:typeof Buffer,Deno:typeof Deno,Bun:typeof Bun,WebAssembly:typeof WebAssembly});`});
...
test('constructor and eval escapes stay inside the guest realm', async t => {
  const root = await project(t,{'/':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`export default (request,context) => {
...
test('dynamic import cannot recover host modules, including code constructed at runtime', async t => {
  const root = await project(t,{'/':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`export default async () => { const load = Function("return import('node:fs')"); await load(); return new Response('escaped'); }`});
...
test('new invocation heap prevents state and prototype pollution crossing requests', async t => {
  const root = await project(t,{'/':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`export default () => { const previous = Object.prototype.infected || null; Object.prototype.infected = 'secret'; globalThis.count = (globalThis.count || 0) + 1; return Response.json({previous,count:globalThis.count}); }`});
...
test('malicious allocations fail within guest limit and redirects survive', async t => {
  const root = await project(t,{'/oom':{sandbox:true,function:{source:'f.mjs'}},'/go':redirect()},{'f.mjs':`export default () => { const values=[]; for(let i=0;i<10000000;i++) values.push({i,data:'x'.repeat(1000)}); return new Response('bad'); }`});
...
test('YAML cannot self-grant secrets; grants are route-scoped and revision-pinned', async t => {
  const root = await project(t,{'/allowed':{sandbox:true,function:{source:'f.mjs'},secrets:{KEY:{secret:'token'}}},'/other':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`import {value} from './helper.mjs'; export default (_request,context) => Response.json({keys:Object.keys(context.secrets),value});`,'helper.mjs':`export const value = 'ok';`});
...
test('capability inspection never executes module top-level code', async t => {
  const root = await project(t,{'/':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`while (true) {} export default () => new Response('no');`});
  const loaded = await loadDocument(root);
...
test('large ArrayBuffer is rejected inside the guest rather than allocated on the host', async t => {
  const root = await project(t,{'/':{sandbox:true,function:{source:'f.mjs'}}},{'f.mjs':`export default () => { try { new ArrayBuffer(256 * 1024 * 1024); return new Response('allocated'); } catch { return new Response('blocked'); } }`});
...
test('runtime-constructed imports cannot read another route module outside the declared graph', async t => {
  const root = await project(t,{'/a':{sandbox:true,function:{source:'a.mjs'}},'/b':{sandbox:true,function:{source:'b.mjs'}}},{'a.mjs':`export default async () => { const load = Function("return import('/b.mjs')"); await load(); return new Response('escaped'); }`,'b.mjs':`export default () => new Response('b');`});
...
test('repeated guest deadlines shed load but never disable functions permanently', async t => {
  const root = await project(t,{'/slow':{sandbox:true,function:{source:'slow.mjs'}},'/fast':{sandbox:true,function:{source:'fast.mjs'}}},{
