...
test('context summarizes the cookbook from the compiled project and the capability catalog',async()=>{
 ...
 assert.deepEqual(context.project.handlers,{redirect:8,respond:19,page:3,static:1,download:2,function:7});
 ...
 assert.equal(context.project.files?.functions.length,7);assert.ok(context.project.files?.functions.includes('functions/hello.mjs'));
