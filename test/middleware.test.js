...
import { project,request,redirect,approveBindings } from './helpers.js';
...
test('middleware wraps function in declared/reverse order and shares only request-local state',async t=>{
 const {app}=await serve(t,{'/f':{middleware:[{source:'a.mjs'},{source:'b.mjs',export:'wrap'}],function:{source:'f.mjs'}}},{
...
test('middleware can short-circuit without invoking downstream code and can recover downstream errors',async t=>{
 const {app}=await serve(t,{'/deny':{middleware:[{source:'deny.mjs'}],function:{source:'loop.mjs'}},'/recover':{middleware:[{source:'catch.mjs'}],function:{source:'throw.mjs'}}},{
...
test('native redirects and binary assets retain their bytes and metadata through middleware',async t=>{
 ...
 const {app}=await serve(t,{'/go':{...redirect(),middleware:wrap()},'/file':{download:{file:'public/a.bin'},middleware:wrap()},'/assets/*':{static:{directory:'public'},middleware:wrap()}},
...
test('native metadata changes, double next and missing Response fail closed',async t=>{
 const {app}=await serve(t,{
  '/metadata':{...redirect(),middleware:[{source:'metadata.mjs'}]},
  '/twice':{...redirect(),middleware:[{source:'twice.mjs'}]},
  '/empty':{...redirect(),middleware:[{source:'empty.mjs'}]}
...
test('one deadline bounds the entire middleware chain; plain redirects remain native',async t=>{
 const {app}=await serve(t,{'/slow':{...redirect(),middleware:[{source:'slow.mjs'},{source:'slow.mjs'}]},'/fast':redirect()},
...
test('middleware shares sandbox restrictions and cannot import unrelated route modules',async t=>{
 const {app}=await serve(t,{'/f':{middleware:[{source:'mw.mjs'}],respond:{text:'native'}},'/other':{function:{source:'secret.mjs'}}},{
 ...
 const root=await project(t,{'/x':{...redirect(),middleware:[{source:'unsafe.mjs'}]}},{'unsafe.mjs':'import fs from "node:fs";export default()=>new Response(fs.readFileSync("/etc/passwd"))'});await assert.rejects(createRuntime(root));
});
test('middleware edits invalidate binding grants and failed reload preserves the old snapshot',async t=>{
 const root=await project(t,{'/go':{...redirect(),middleware:[{source:'mw.mjs'}],secrets:{KEY:{secret:'key'}}}},{'mw.mjs':'export default async (req,ctx,next) => {const res=await next();res.headers.set("x-value",ctx.secrets.KEY);return res;}'});
...
test('readiness does not assume middleware-wrapped native routes behave like plain redirects',async t=>{
 const {app}=await serve(t,{'/go':{...redirect(),middleware:[{source:'mw.mjs'}]}},{'mw.mjs':'export default (req,ctx,next)=>next()'});
...
test('middleware respects validation, final YAML headers and explicit native response replacement',async t=>{
 const {app}=await serve(t,{'/go':{...redirect(),middleware:[{source:'replace.mjs'}],response:{headers:{'x-final':'yaml'}},parameters:[{name:'key',in:'query',required:true,schema:{type:'string'}}]}},
...
test('missing middleware exports reject startup and next arguments fail closed',async t=>{
 const root=await project(t,{'/go':{...redirect(),middleware:[{source:'mw.mjs',export:'missing'}]}},{'mw.mjs':'export default (req,ctx,next)=>next()'});
 await assert.rejects(createRuntime(root));
 const {app}=await serve(t,{'/go':{...redirect(),middleware:[{source:'mw.mjs'}]}},{'mw.mjs':'export default (req,ctx,next)=>next("unsupported")'});
