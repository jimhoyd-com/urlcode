...
test('MCP review_project flags hand-written body validation without executing project code',async t=>{
 const root=await project(t,{'/manual':{sandbox:true,methods:['POST'],function:{source:'v.mjs'}}},{'v.mjs':"export default function(request){ const body=request.body; if(typeof body!=='object')throw new Error('bad'); return new Response('ok'); }"});
 ...
 const report=JSON.parse(replies[1]!.result.content[0]!.text);assert.equal(report.format,1);assert.equal(report.findings[0].pattern,'manual-body-validation');
});
