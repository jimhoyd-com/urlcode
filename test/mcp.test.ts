...
test('MCP negotiates explicit supported protocol and lists read-only implemented tools',async t=>{
 ...
 assert.equal(replies[0]!.result.protocolVersion,'2025-11-25');assert.equal(replies[1]!.result.tools.length,10);assert.equal(JSON.parse(replies[2]!.result.content[0]!.text).routeCount,1);
});
