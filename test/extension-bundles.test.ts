...
test('BUNDLE_CATALOG_NAMES lists every first-party bundle this release builds',()=>{
  assert.deepEqual(BUNDLE_CATALOG_NAMES.map(item=>item.name).sort(),['admin','auth','forms','mcp','store','ui','ui-presentation']);
...
test('bundle names are checked locally, with a suggestion, before the GitHub transport touches the network (#579)',async t=>{
  ...
  await assert.rejects(()=>installBundle(project,'extension-bundles@v1.0.0','auht'),/Unknown extension bundle auht; did you mean auth\? Known bundles: ui, ui-presentation, auth, admin, store, forms, mcp/);
  ...
  assert.doesNotThrow(()=>assertKnownBundleNames(['ui','ui-presentation','auth','admin','store','forms','mcp']));
