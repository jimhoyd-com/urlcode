...
function archive(){const manifest={format:1,coreVersion:'0.4.8',bundles:[{name:'sample',version:'1.2.3',entry:modulePath}]};return tar({'bundle.json':JSON.stringify(manifest),[modulePath]:'export const loaded = "verified";'});}

test('extension bundle extraction accepts a frozen module tree and rejects extra files',async t=>{
  const project=await mkdtemp(join(tmpdir(),'urlcode-bundle-'));t.after(async()=>{await import('node:fs/promises').then(fs=>fs.rm(project,{recursive:true,force:true}));});const bytes=archive(),item={...entry(bytes),coreVersion:'0.4.8'};
  ...
  const invalid=tar({'bundle.json':JSON.stringify({format:1,coreVersion:'0.4.8',bundles:[{name:'sample',version:'1.2.3',entry:modulePath}]}),[modulePath]:'export{}','README.md':'not part of a module tree'});
  await assert.rejects(()=>extractBundle(invalid,{...entry(invalid),coreVersion:'0.4.8'},join(project,'invalid')),/outside its frozen module tree/);
});

test('signed bundle installation writes a lock and host loader imports only the verified entry',async t=>{
  const project=await mkdtemp(join(tmpdir(),'urlcode-bundle-install-'));t.after(async()=>{await import('node:fs/promises').then(fs=>fs.rm(project,{recursive:true,force:true}));});const bytes=archive(),item=entry(bytes),catalog=Buffer.from(JSON.stringify({format:1,tag:'extension-bundles@v1.0.0',commit:'a'.repeat(40),coreVersion:'0.4.8',bundles:[item],revoked:[]})),verified:string[]=[];
...
test('bundle catalog refuses tag changes, duplicate names and a core mismatch',()=>{
  const item={...entry(Buffer.from('bundle')),sha256:'b'.repeat(64)},base={format:1,tag:'extension-bundles@v1.0.0',commit:'a'.repeat(40),coreVersion:'0.4.8',bundles:[item],revoked:[]};
