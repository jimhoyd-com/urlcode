...
test('skills and AI-AUTHORING agree on the bounded-first order', async () => {
  for (const path of ['skills/urlcode/SKILL.md', '.claude/skills/urlcode-authoring/SKILL.md', 'packaging/claude-plugin/skills/urlcode-authoring/SKILL.md']) {
    ...
    assert.match(text, /get_extension_artifacts/, path);
    assert.match(text, /get_extension_artifact/, path);
  }
  ...
  for (const need of ['get_context', 'urlcode context --project DIR', '--budget', 'capabilities NAME', 'get_schema', 'search_recipes', 'explain', 'get_extensions', 'get_extension_artifacts', 'get_extension_artifact', 'llms-full.txt', 'fallback', 'never hides a']) assert.ok(section.includes(need), need);
});
