...
test('every member of the scaffold contract is either baseline or has a recorded core release', async () => {
  ...
  assert(members.includes('allowPublicWrite') && members.includes('name'), 'the contract interfaces were not found');
...
test('the scan reads property names, not comments or strings', () => {
  const names = propertyNames("// after: comment\nconst text = 'requires: string';\nconst a = { provides: [], routeNotes };\nconst { allowPublicWrite, names: n } = request; request.publicWrite;\nimport { ExtensionAuthoringContract as C } from 'x';");
  assert.deepEqual([...names].sort(), ['ExtensionAuthoringContract', 'allowPublicWrite', 'names', 'provides', 'publicWrite', 'routeNotes']);
...
test('the store scaffold and its authoring contract need a core floor above 0.4.2 (#346)', async () => {
  ...
  for (const field of ['allowPublicWrite', 'publicWrite', 'routeNotes', 'provides', 'after', 'authoring']) assert(used.some(use => use.field === field), `${field} not detected`);
  ...
  await assert.rejects(assertPeerFloorCoversApi(root, 'packages/store', '@jimhoyd/urlcode-store', { [coreName]: '>=0.4.2 <0.5.0' }), /allowPublicWrite/);
...
test('a peer floor is raised to the needed core release, keeping its upper bound, and only when core has it', () => {
  const uses = [{ field: 'allowPublicWrite', since: '0.4.3' }, { field: 'provides', since: '0.4.3' }];
