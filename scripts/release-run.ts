...
export function options(args: string[]): Options {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--execute') result.execute = true;
      ...
      if (arg === '--version') { assert(semver.valid(value) === value && /^\d+\.\d+\.\d+-alpha\.\d+$/.test(value), 'Use an explicit alpha version'); result.version = value; }
