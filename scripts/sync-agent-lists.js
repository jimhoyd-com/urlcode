...
//
//   node scripts/sync-agent-lists.js            # fetch, validate, write
//   node scripts/sync-agent-lists.js --check    # exit 1 when files would change
//
...
async function loadValidator() {
  ...
  return (await import('../src/policies/agents.js')).validatePattern;
}
...
async function main() {
  ...
  if (check && changed) { console.error(`${changed} file(s) out of date; run node scripts/sync-agent-lists.js`); process.exit(1); }
