...
// urlcode.json and ordered so every add-on follows the ones it requires. Scripts, CI and the release all use this
...
export interface Addon { name: string; kind: 'extension' | 'artifact'; directory: string; packageName: string; version: string; description: string; requires: string[]; scripts: Record<string, string> }
...
export async function addons(root = repositoryRoot): Promise<Addon[]> {
  ...
  for (const parent of ['packages', 'artifacts']) {
    ...
    for (const name of names.sort()) {
      ...
      let descriptor: { kind?: unknown; name?: unknown; description?: unknown; requires?: unknown };
      ...
      found.push({ name, kind, directory, packageName: pkg.name, version: pkg.version, description: String(descriptor.description ?? ''), requires: Array.isArray(descriptor.requires) ? descriptor.requires.map(String) : [], scripts: pkg.scripts ?? {} });
    }
  }
