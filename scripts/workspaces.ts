...
// urlcode.json and ordered so every add-on follows the ones it requires, and the ones it contributes to (an optional
// edge: a contributor's tests may compose with its target, so the target is built first). Scripts, CI and the release all use this
...
export interface Addon { name: string; kind: 'extension' | 'artifact'; directory: string; packageName: string; version: string; description: string; requires: string[]; contributes: string[]; scripts: Record<string, string> }
...
export async function addons(root = repositoryRoot): Promise<Addon[]> {
  ...
  for (const parent of ['packages', 'artifacts']) {
    ...
    for (const name of names.sort()) {
      ...
      let descriptor: { kind?: unknown; name?: unknown; description?: unknown; requires?: unknown; contributes?: unknown };
      ...
      found.push({ name, kind, directory, packageName: pkg.name, version: pkg.version, description: String(descriptor.description ?? ''), requires: Array.isArray(descriptor.requires) ? descriptor.requires.map(String) : [], contributes: Array.isArray(descriptor.contributes) ? descriptor.contributes.map(String) : [], scripts: pkg.scripts ?? {} });
    }
  }
  ...
  while (ordered.length < found.length) {
    const ready = found.filter(addon => !placed.has(addon.name) && addon.requires.every(requirement => { if (!byName.has(requirement)) throw new Error(`${addon.name} requires unknown add-on ${requirement}`); return placed.has(requirement); })
      && addon.contributes.every(target => !byName.has(target) || placed.has(target)));
