...
export function gate(plan: string, results: Record<string, { result: string }>, workspaceIntegration = false, action = false, buildFidelity = false, container = false, packageFloorSmoke = false): void {
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv[2] === 'gate') {
    gate(process.env.CI_PLAN ?? '', JSON.parse(process.env.CI_RESULTS ?? '{}'), process.env.CI_WORKSPACE_INTEGRATION === 'true', process.env.CI_ACTION === 'true', process.env.CI_BUILD_FIDELITY === 'true', process.env.CI_CONTAINER === 'true', process.env.CI_PACKAGE_FLOOR_SMOKE === 'true');
