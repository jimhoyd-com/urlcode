...
test('HTTP email callbacks receive scoped locales and known-account notices prefer saved locale without public enumeration',async t=>{
 ...
 const existing=await request('/forgot-password?lang=fr',{email:user.user.email},csrf),count=messages.length,unknown=await request('/forgot-password?lang=fr',{email:'unknown@example.test'},csrf);assert.equal(existing.status,unknown.status);assert.deepEqual(existing.json<MessageBody>(),unknown.json<MessageBody>());assert.equal(messages.length,count);assert.equal(messages.at(-1)?.purpose,'reset-password');assert.equal(messages.at(-1)?.locale,'fr');
