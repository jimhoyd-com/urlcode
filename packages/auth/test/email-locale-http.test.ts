...
test('HTTP email callbacks receive scoped locales and known-account notices prefer saved locale without public enumeration',async t=>{
 ...
 const verified=await service.issueToken({email:user.user.email,purpose:'verify-email'});await service.consumeVerification(verified.token!);cookies.clear();csrf=(await request('/csrf')).json<CsrfBody>().csrf;
