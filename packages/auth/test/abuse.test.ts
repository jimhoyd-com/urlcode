...
test('password backoff persists across restart, resets on success and verified reset never bypasses MFA',async t=>{
 const root=await mkdtemp(join(tmpdir(),'auth-abuse-'));removeAtExit(root);let clock=1800000000000;const options={database:join(root,'auth.sqlite'),encryptionKey:randomBytes(32),roles:{member:['site.read']},defaultRole:'member',now:()=>clock,abuse:{passwordBackoff:{threshold:2,initialDelayMs:1000,maxDelayMs:8000,resetAfterMs:60000}}};
...
test('velocity budgets are atomic per trusted client and signup domain, durable and expiring',async t=>{
 const root=await mkdtemp(join(tmpdir(),'auth-velocity-'));removeAtExit(root);let clock=1800000000000;const options={database:join(root,'auth.sqlite'),encryptionKey:randomBytes(32),roles:{member:[]},defaultRole:'member',now:()=>clock,abuse:{client:{limit:4,windowMs:10000},signupClient:{limit:2,windowMs:10000},signupDomain:{limit:2,windowMs:10000},challengeAfter:1}};
