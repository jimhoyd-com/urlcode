...
export function scaffold(request: ScaffoldRequest): ScaffoldResult {
  ...
  if (withAuth && request.names.indexOf('auth') > request.names.indexOf('store')) throw new Error('List auth before store so the shared revision pin is defined first: urlcode init --with auth,store');
  return {
    name: 'store',
    ...
    hostSetup: [
      ...(withAuth ? [] : ['const projectSha256 = process.env.PROJECT_SHA256;', "if (!projectSha256 || !/^[a-f0-9]{64}$/.test(projectSha256)) throw new Error('Set the reviewed PROJECT_SHA256 revision');"]),
      "const storeDirectory = process.env.STORE_DIRECTORY ?? fileURLToPath(new URL('./data/store', import.meta.url));",
    ],
    hostEntries: ['storeExtension({directory: storeDirectory, projectSha256})'],
    files: [],
    readme,
