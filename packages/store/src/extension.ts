...
function example(request: ScaffoldRequest): ScaffoldResult {
  ...
  return {
    ...
    notes: [
      withAuth ? 'store serves /api/todos to signed-in callers only (auth: {csrf: origin} on the mount: JSON writes are admitted on same-origin provenance and the session cookie), and each user sees and changes only their own todos (ownership: owner).' : 'store serves /api/todos with public write: anyone who can reach the server can change records. Add auth and `auth: true` on the mount to protect it.',
