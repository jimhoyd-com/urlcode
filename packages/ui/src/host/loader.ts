 ...
 * dedicated ui/ directory after symlink resolution; sizes and counts are bounded.
 */
...
async function presentationPath(root: string, path: string, what: string): Promise<string> {
    relativePath(path, what);
    if (!path.startsWith('ui/')) throw new Error('ui paths must stay inside the project ui/ presentation directory');
    const directory = join(root, 'ui');
    if (await realpath(directory) !== directory) throw new Error('ui presentation directory must not be a symlink');
    return inside(directory, path.slice(3));
}
...
export async function loadProjectUi(projectRoot: string, config: UiConfig): Promise<ProjectUi> {
    ...
    if (config.copy !== undefined) {
        const dir = await presentationPath(root, config.copy, 'copy');
        for (const language of languages) {
            const file = join(dir, `${language}.json`);
            let source: string;
            try { source = await readBounded(await inside(join(root, 'ui'), relative(join(root, 'ui'), file)), limits.catalogueBytes); }
            catch (error) { if (language === 'en' && (error as NodeJS.ErrnoException).code === 'ENOENT') continue; throw error; }
    ...
    if (config.templates !== undefined) {
        const dir = await presentationPath(root, config.templates, 'templates');
        const files: string[] = [];
        ...
        for (const file of files.sort()) {
            ...
            templates[name] = await readBounded(await inside(join(root, 'ui'), relative(join(root, 'ui'), join(dir, file))), limits.templateBytes);
        }
    }
    ...
    if (config.stylesheet !== undefined) {
        ...
        const file = await presentationPath(root, spec.file, 'stylesheet');
