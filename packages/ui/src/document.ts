...
export interface DocumentOptions {title:string;trustedContent:string;layout?:'default'|'compact'|'application';theme?:{nonce:string};style?:{nonce:string};presentation?:PresentationContext;scripts?:readonly {src:string;nonce:string;async?:boolean}[]}
const nonceShape=/^[A-Za-z0-9+/_=-]{16,128}$/;
/**
 * The strict Content-Security-Policy a `renderDocument` page needs: inline style
 * and script only by the per-response nonce, everything else same-origin. The
 * default `oshp` profile lets a response's own CSP win, so a function returns this
 * as its `content-security-policy` header and passes the same nonce as
 * `style: {nonce}` (and `theme: {nonce}`); the profile itself is unchanged.
 * Generate a fresh unpredictable nonce for every response.
 */
export function documentContentSecurityPolicy(nonce:string):string {
 if(!nonceShape.test(nonce))throw new Error('Invalid nonce');
 return `default-src 'none'; style-src 'self' 'nonce-${nonce}'; script-src 'nonce-${nonce}'; img-src 'self'; font-src 'self'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'`;
}
...
export function renderDocument(options:DocumentOptions):string {
 ...
 if(options.style&&!nonceShape.test(options.style.nonce))throw new Error('Invalid style nonce');
