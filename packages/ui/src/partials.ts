...
{{#each scripts}}<script nonce="{{nonce}}" src="{{href src}}"{{#if integrity}} integrity="{{integrity}}"{{/if}} defer></script>{{/each}}</body></html>`,
    ...
    textarea: {
        source: `{{!-- viewModel: textarea@1 --}}<div class="ui-field{{#if error}} ui-field-invalid{{/if}}"><label class="ui-label" for="{{id}}">{{label}}{{#if required}}{{else}} <span class="ui-muted">({{t "ui.field.optional"}})</span>{{/if}}</label><textarea class="ui-input ui-textarea" id="{{id}}" name="{{name}}" rows="{{rows}}"{{#if placeholder}} placeholder="{{placeholder}}"{{/if}}{{#if required}} required{{/if}} maxlength="{{maxlength}}"{{#if help}} aria-describedby="{{id}}-help"{{/if}}{{#if error}} aria-invalid="true" aria-errormessage="{{id}}-error"{{/if}}>
{{value}}</textarea>{{#if help}}<p class="ui-help" id="{{id}}-help">{{help}}</p>{{/if}}{{#if error}}<p class="ui-error" id="{{id}}-error" role="alert">{{error}}</p>{{/if}}</div>`,
    ...
    select: {
        source: `{{!-- viewModel: select@1 --}}<div class="ui-field{{#if error}} ui-field-invalid{{/if}}"><label class="ui-label" for="{{id}}">{{label}}{{#if required}}{{else}} <span class="ui-muted">({{t "ui.field.optional"}})</span>{{/if}}</label><select class="ui-input ui-select" id="{{id}}" name="{{name}}"{{#if required}} required{{/if}}{{#if help}} aria-describedby="{{id}}-help"{{/if}}{{#if error}} aria-invalid="true" aria-errormessage="{{id}}-error"{{/if}}>{{#if placeholder}}<option value="">{{placeholder}}</option>{{/if}}{{#each options}}<option value="{{value}}"{{#if selected}} selected{{/if}}{{#if disabled}} disabled{{/if}}>{{label}}</option>{{/each}}</select>{{#if help}}<p class="ui-help" id="{{id}}-help">{{help}}</p>{{/if}}{{#if error}}<p class="ui-error" id="{{id}}-error" role="alert">{{error}}</p>{{/if}}</div>`,
