 ...
 * action release. Any failure restores package.json, package-lock.json and the workflows, and reinstalls.
 */
export async function upgradeSite(directory: string, options: { to?: string | undefined } = {}): Promise<UpgradeResult> {
  ...
  try {
    ...
    await state.restore();
    await runNpm(['install', '--ignore-scripts', '--no-audit', '--no-fund'], site.site).catch(() => undefined);
    throw error;
  }
}
