    ...
    try {
      ...
      for (const [alias, ref] of Object.entries(config.env || {})) {
        ...
        if (ref.env) {
          ...
          assert(granted || ref.default !== undefined, `Environment binding denied by operator policy: ${alias} reads ${ref.env}, which no grant allows for this route. ${grantHint}; or declare a default${revisionPinHint(permissions.projectSha256, projectSha256)}`, { code: 'binding-denied' });
      ...
      for (const [alias, ref] of Object.entries(config.secrets || {})) {
        assert(permissions.projectSha256 === projectSha256 && permissions.routes?.[pattern]?.secrets?.includes(ref.secret), `Secret binding denied by operator policy: ${alias} reads ${ref.secret}, which no grant allows for this route. ${grantHint}${revisionPinHint(permissions.projectSha256, projectSha256)}`, { code: 'binding-denied' });
