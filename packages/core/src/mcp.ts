...
export async function serveMcp(options:McpOptions):Promise<void> {
 ...
 const line=async(bytes:Buffer)=> {
  ...
  if(message.method==='initialize') {
   ...
   initialized=true;await send({jsonrpc:'2.0',id,result:{protocolVersion:negotiateProtocolVersion(params.protocolVersion),capabilities:{tools:{}},serverInfo:{name:'urlcode',version:'0.6.2'}}});return;
  }
