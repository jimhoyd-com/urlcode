...
import { AUDIT_APPLICATION_ID, patched } from './store.ts';
...
export interface BackupResult {
    format: 'urlcode-audit-sqlite-v1';
    bytes: number;
}
...
async function snapshot(sourceInput: string, destinationInput: string, projectRoot: string): Promise<BackupResult> {
    ...
    if (!outside(project, source) || !outside(project, destination))
        throw new Error('Audit backups and databases must be outside the project');
    ...
    try {
        ...
        await created.close();
        worker = new Worker(new URL(import.meta.url), { workerData: { urlcodeAuditBackup: true, source, destination: file }, env: {}, execArgv: [], stdout: true, stderr: true, resourceLimits: { maxOldGenerationSizeMb: 32 } });
        ...
        return { format: 'urlcode-audit-sqlite-v1', bytes: copied.size };
    }
    finally {
...
/** Restores to a new isolated path only; stop the host and move the restored file into place yourself. */
...
if (!isMainThread && workerData?.urlcodeAuditBackup) {
    ...
    try {
        ...
        if (source.prepare('PRAGMA user_version').get()?.user_version !== 1 || source.prepare('PRAGMA application_id').get()?.application_id !== AUDIT_APPLICATION_ID)
            throw new Error('Unsupported audit database');
        const schema = source.prepare("SELECT type,name FROM sqlite_master WHERE name NOT LIKE 'sqlite_%'").all();
        if (schema.some(row => !['table', 'index'].includes(String(row.type)) || !String(row.name).startsWith('audit_')))
