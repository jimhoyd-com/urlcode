...
export async function scaffold(request: ScaffoldRequest): Promise<ScaffoldResult> {
    ...
    // Admin renders through the ui kit too, and the runtime activates extensions in declaration order.
    const uiPosition = request.names.indexOf('ui'), adminPosition = request.names.indexOf('admin');
    if (uiPosition < 0)
        throw new Error("Admin scaffold requires the ui extension: urlcode init --with ui,auth,admin");
    if (adminPosition >= 0 && uiPosition > adminPosition)
        throw new Error("Admin scaffold requires the ui extension before admin: urlcode init --with ui,auth,admin");
    return {
        name: 'admin',
        ...
        hostSetup: ['// Admin reuses service, csrfKey, projectSha256 and the ui extension from the setup above.'],
        hostEntries: ["adminExtension({service, csrfKey, projectSha256, ui, authMount: '/account'})"],
        files: [],
        readme,
...
export async function initAdministration(directory: string): Promise<{directory:string;project:string;hostFile:string;operatorFile:string}> {
    ...
    try {
        const request = { directory: created.directory, project: created.project, hostFile: created.hostFile, names: ['ui', 'auth', 'admin'] };
        const ui = await uiScaffold(request), admin = await scaffold(request);
        const document = { version: '1', extensions: { ...ui.extensions, auth: { version: '1', config: { registration: 'off' } }, ...admin.extensions }, routes: { ...ui.routes, '/account/*': { extension: 'auth', methods: ['GET', 'HEAD', 'POST'] }, ...admin.routes, '/private': { respond: { text: 'Signed in' }, policies: { extensions: { auth: {} } } } } };
