...
const screens:Record<string,{body:string;sample:ViewModel}>={
 dashboard:{
  body:`{{#if impersonation}}<details class="ui-card ui-danger-zone" data-slot="card"><summary>{{impersonation.summary}}</summary>{{impersonation.form}}</details>{{/if}}{{#if summary}}{{summary}}{{/if}}{{#if activity}}<details class="ui-card ui-dashboard-details" data-slot="card"><summary id="daily-heading">{{activity.heading}}</summary><div class="ui-card-content" data-slot="card-content">${activityTable('dailyCaption','days','dayHeading','day')}${activityTable('methodsCaption','methods','methodHeading','method')}</div></details>{{/if}}{{#if recent}}<section class="ui-card ui-recent-card" data-slot="card"><header class="ui-card-header ui-section-heading" data-slot="card-header"><h2 class="ui-card-title" data-slot="card-title">{{recent.heading}}</h2><a class="ui-button ui-button-ghost" data-slot="button" href="{{href recent.auditHref}}">{{recent.auditLabel}}</a></header><div class="ui-card-content" data-slot="card-content"><ul class="ui-activity">{{#if recent.events}}{{#each recent.events}}<li><code>{{action}}</code><time class="ui-muted" datetime="{{datetime}}">{{label}} UTC</time></li>{{/each}}{{else}}<li><div class="ui-empty" data-slot="empty"><p class="ui-empty-title" data-slot="empty-title">{{recent.empty}}</p></div></li>{{/if}}</ul></div></section>{{/if}}`,
