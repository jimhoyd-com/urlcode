...
import type {AuthAccount,AuthSession,AuthUser} from '@jimhoyd/urlcode-auth';
...
export function accountDetail(input:{user:AuthUser;account:AuthAccount;mount:string;csrf:string;copy:AdminCopy;sessions?:AuthSession[];activity?:AuditPage;notes?:AuditPage;operations:boolean;recovery?:boolean}):Screen {
 const {user,account,mount,csrf,copy,sessions,activity,notes}=input;
 const plain=(source:string)=>copy.s(source),text=(source:string)=>escapeHtml(plain(source)),value=escapeHtml;
 const url=(path:string)=>escapeHtml(mount+path),id=encodeURIComponent(user.id),field=(name:string,label:string)=>formField(name,plain(label));
 const form=(path:string,fields:string,label:string,destructive=false)=>postForm({action:mount+path,csrf,fields:hiddenField('accountId',user.id)+fields,label:plain(label),destructive,className:'ui-form-grid'});
 const sections=[['overview','Overview'],['methods','Sign-in methods'],['sessions','Sessions'],['recovery','Recovery'],['activity','Activity'],['data','Data']] as const;
 const empty=(message:string)=>emptyState(plain(message));
 ...
 const operations=input.operations&&user.id!==account.id?`<a class="ui-button-secondary" href="${url('/account-operations?accountId='+id)}">${text('Account administration')}</a>`:'';
 const overview=`<dl class="ui-definition-grid">${facts.map(([name,content])=>`<dt>${text(name!)}</dt><dd>${value(content!)}</dd>`).join('')}</dl>`+(account.has('auth.users.reveal')?`<details><summary>${text('Reveal email address')}</summary>${form('/users/reveal',field('reason','Reason'),'Reveal email address')}</details>`:'')+(notes?`<h3>${text('Administrator notes')}</h3>${notes.events.length?`<ul class="ui-list">${notes.events.map(note=>`<li><p>${value(note.reason)}</p><small class="ui-muted">${value(note.actor)} · ${adminTime(note.at)}</small></li>`).join('')}</ul>`:empty('No administrator notes yet.')}`:'')+(account.has('auth.users.manage')?`<details><summary>${text('Save administrator note')}</summary><p class="ui-muted">${text('Notes are recorded in the audit trail. Do not include credentials or recovery secrets.')}</p>${form('/users/note',field('reason','Administrator note'),'Save administrator note')}</details>`:'');
