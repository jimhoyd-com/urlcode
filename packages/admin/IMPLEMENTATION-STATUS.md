...
alignment](../../docs/VERSION-ALIGNMENT.md) records it. Core, ui, auth and admin are
...
- Embedded hosts can use `createAdministrationRuntime` to install the support banner and live runtime health together; an actual-runtime regression verifies cached application pages, impersonation, revocation and admin denial. Every request must use the returned runtime. Arbitrary frontend scripts remain outside the trusted UI guarantee.
