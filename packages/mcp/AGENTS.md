...
- Run `npm run verify` for every change. The request-id fidelity, protocol
  ...
  test in the same PR (`test/mcp.test.ts`); scaffold changes need one in
  `test/scaffold.test.ts` and, for `init --with mcp` composition, the
  real coverage in the root `test/workspace-scaffold.integration.ts`.
