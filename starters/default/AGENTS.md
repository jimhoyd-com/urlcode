...
When present, `.mcp.json` registers the read-only `urlcode mcp` server for Claude Code, which never executes project code (Codex does not read that file: its operator registers the same command under `[mcp_servers.urlcode]` in `~/.codex/config.toml` or a trusted project `.codex/config.toml`, with this site as `cwd`, or with `codex mcp add`); prefer its
