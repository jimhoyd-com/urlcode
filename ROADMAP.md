...
1. Select an open-source license; define the versioned YAML schema, HTTP input
...
3. Add CSV/YAML/JSON import/export, list/filter, safe bulk update, duplicate and
   redirect-loop checks, file composition and reusable pinned templates.
4. Test real HTTP behavior and asynchronous signals with local fake receivers;
   add optional ngrok lifecycle integration without requiring it for tests.
5. Ship generic self-hosting and provider adapters incrementally, documenting
...
6. Provide Homebrew and tested macOS/Linux/Windows distribution, clean-install
